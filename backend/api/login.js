...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    const { user_id: fetchedUserID, username: fetchedUsername } =
    ...
    if (
      SuspensionState.rows[0].suspended === true &&
      ...
      if (suspendedUntil <= formattedDate) {
        //time to unban
      ...
      SuspensionState.rows[0].suspended === false &&
      userID === fetchedUserID
    ) {
      //no banned
