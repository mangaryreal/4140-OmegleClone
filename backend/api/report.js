...
router.post("/report", async (request, response) => {
  ...
  try {
    ...
    const reporterResult = await client.query(fetchReporterUserIDQuery, [
      reporter_name,
    ]);
    const reporter_id = reporterResult.rows[0].user_id;

    const fetchReportedUserIDQuery = `SELECT USER_ID FROM "User" WHERE USERNAME = $1`;
    const reportedResult = await client.query(fetchReportedUserIDQuery, [
      reported_name,
    ]);
    ...
    const countNoOfReportForUserResult = await client.query(
      countNoOfReportForUserQuery,
      [reported_id]
    );
